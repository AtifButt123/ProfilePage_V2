...
function ProfilePage() {
  ...
  const [friendCount, setFriendCount] = useState(0); // Added friend count state
  ...
  useEffect(() => {
    // Fetch friend count for the selected person
    const fetchFriendCount = async () => {
      try {
        if (selectedPerson) {
          const response = await axios.get(`http://localhost:5000/persons/${selectedPerson.publicKey}/friendCount`);
  ...
  const handlePersonClick = async (personId) => {
    try {
      ...
      // Checking if a friend is already in friend request
        ...
        <div className="col-sm-12 col-md-6 my-2 py-2">
          ...
          >
            ...
            <Post
              ...
              userNickName={selectedPerson?.nickName}
